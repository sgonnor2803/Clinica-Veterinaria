...
function showApp() {
    ...
    const isAdmin = appState.profile?.role === 'admin' || appState.profile?.role === 'sales' || appState.profile?.role === 'vet';
...
async function loadDashboardData() {
    try {
        ...
        if (appState.profile?.role === 'vet' || appState.profile?.role === 'admin') {
            ...
            appointments = await api.getMyAppointments().catch(() => []);
            orders = await api.getOrders().catch(() => []);
        }
        ...
        document.getElementById('appointmentsCount').textContent = `${appointments.length} citas`;
        document.getElementById('ordersCount').textContent = `${orders.length} órdenes`;
    } catch (error) {
    ...
    try {
        ...
        container.innerHTML = products.map(product => `
            <div class="product-card">
                <div class="card-body">
                    <h3>${product.name}</h3>
                    <p>${product.type || 'Producto'}</p>
                    ${product.discountApplied ? `
                        <div class="card-discount">¡Descuento 20% por adopción!</div>
...
async function loadPets() {
    try {
        ...
        if (unadopted.length === 0) {
            container.innerHTML = '<p class="text-center">No hay mascotas disponibles</p>';
            return;
        }
        ...
        container.innerHTML = unadopted.map(pet => `
            <div class="pet-card">
                <div class="card-body">
                    ...
                    <p style="color: var(--success); font-weight: 600;">Disponible</p>
                    <div class="card-actions">
                        <button class="btn btn-success" onclick="window.adoptPet(${pet.id}, '${pet.name}')">
                            Adoptar
                        </button>
                    </div>
                </div>
        ...
        if (!appointments || appointments.length === 0) {
            ...
            container.innerHTML = appointments.map(apt => `
                <div class="appointment-card">
                    <div class="card-body">
                        <h3>📅 ${apt.date}</h3>
                        <p>${apt.description || 'Sin descripción'}</p>
...
// ============ Admin Functions ============
window.showAddProductForm = () => showModal('productModal');
window.showAddPetForm = () => showModal('petModal');
